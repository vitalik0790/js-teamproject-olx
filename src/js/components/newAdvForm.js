...
export const newAdvFormComponent = (advData) => {
    ...
    const createSecondBlock = () => {
        ...
        for (let i = newAdv.file.length; i < 4; i += 1) {
            ...
            <label for="file_loader${newAdv.file.length}"   class="file-loader-label"></label>
            </li>`
        }
        return greyWrapp
    }
    const createMarkupCategories = () => {
        let categoriesMarkup = "<option value='' class='select-option'>Выберите категорию</option >"
    ...
    const getPrice = () => {
        if ((newAdv.category === "free" || newAdv.category === "work" || newAdv.category === "trade") && newAdv.price > 0) {
            noticeAlert('Цена должна быть 0 в категориях "Работа","Отдам бесплатно", "Обмен". Данные были откорректированы автоматически').open()
            return 0
        } else {
    ...
    const postNewAdv = async (e) => {
        ...
        if (!advData) {
            ...
            try {
                ...
                const result = await axios.patch(`${baseURL}/${newAdv._id}`, formData, options)
                data.user.ownCall = [...data.user.ownCall.map(item => item._id === newAdv._id ? { ...result.data } : item)]
                await closeModal();
                successAlert("Объявление успешно записано!").open()
            } catch (error) {
                errorAlert("Что-то пошло не так( Попробуйте еще раз").open()
        ...
        const deleteAdv = async (e) => {
            ...
            if (e.target.closest('[data-button="delete-button"]')) {
                try {
                    ...
                    console.log(result);
                    data.user.ownCall = [...data.user.ownCall.filter(item => item._id !== newAdv._id)]
                    await closeModal();
