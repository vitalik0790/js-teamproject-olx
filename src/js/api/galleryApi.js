...
export const init = async () => {
  ...
  const fetcherWithCounter = async (categoriesNum, cardsNum) => {
    ...
    for (let i = 0; i < categoriesNum; i += 1) {
      await axios.get(`${baseURL}/call/specific/${data.categories[categoriesShown]}`).then(async response => await createMarkup(response.data, cardsNum)).catch(error => console.log(error))
      categoriesShown += 1;
      // console.log(categoriesShown);
